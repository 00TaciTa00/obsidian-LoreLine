...
마크다운에서 하고, 이 플러그인은 그것을 다르게 보여 주기만 한다.
...
| 항목 | 기본값 | 설명 |
|------|--------|------|
| 대상 폴더 | (비어 있음 = 볼트 전체) | 노트를 찾을 폴더. 앞뒤 슬래시는 알아서 벗긴다 |
| 정의 파일 경로 | `loreline.config.json` | 볼트 최상위에서부터의 경로 |
...
src/
  ...
  loader/     볼트 → 데이터. 스캔·설정·해소·진단
  view/       데이터 → 화면. 렌더러 셋과 뷰 하나
  testing/    옵시디언 껍데기와 DOM 셰임. 번들에는 안 들어간다
tools/preview/  옵시디언 없이 뷰를 HTML로 뽑는다
```

흐름은 한 방향이다: `scan → config → resolve → render`. 로더가 만든
