...
- 여러 인물·장소에 걸친 사건은 **카드 한 장이 그 열들을 가로지른다.** 카드 위쪽
  띠에는 실제로 걸린 열에만 색이 찍힌다. 같은 행에서 가로로 겹치는 사건은 아래
  단으로 내려간다.
- 인물별 격자에는 **흐름선**이 있다. 그 인물이 처음 나오는 행부터 마지막으로
  나오는 행까지 세로선을 긋고 양끝에 점을 찍는다. 선의 길이가 그 인물이 이야기에
  머무는 구간이다.
...
릴리스에서 받아 넣어도 된다. 세 파일을 위 경로에 두면 똑같다.
...
- 다만 **글자가 정확히 같아야 한 행으로 묶인다.** `"3년째 겨울"`과
  `"3년째 겨울(밤)"`은 다른 행이다.
- 사건에는 따로 색이 없다. 카드의 **왼쪽 띠는 기간 색**, 공간별·인물별 격자에서는
  **위쪽 띠가 그 카드가 놓인 열의 색**이다. 색은 `loreline.config.json`에서 온다.
  사건 노트에 남은 `color:`는 읽지 않는다.
...
`buildEraGroups`, `layoutRow`, `laneLifespans`)는 알고리즘을 그대로 옮겼다. 타입에서 DB 컬럼을 걷어내고 `id`를
이름 문자열로 바꿨고, 그 뒤 이름을 눌러 문서로 가는 기능 때문에 결과에 노트 경로를
더 싣고, 격자의 시간 칸을 기간과 시각으로 나눠 담도록 고쳤다.

## 릴리스

버전은 세 곳(`package.json`, `manifest.json`, `versions.json`)이 같아야 한다.
`npm version`이 맞춰 준다.

```bash
npm version patch      # 세 파일을 함께 고치고 커밋·태그까지 만든다
git push --follow-tags
```

태그를 밀면 GitHub Actions가 검사·빌드를 돌리고 `main.js`·`manifest.json`·
`styles.css`를 붙인 릴리스를 만든다. **태그 이름은 버전과 정확히 같아야 한다** —
`v0.1.0`처럼 접두사를 붙이면 옵시디언이 찾지 못한다.

## 라이선스

MIT. 원본 [LoreLine](https://github.com/00TaciTa00/LoreLine)을 따른다.
